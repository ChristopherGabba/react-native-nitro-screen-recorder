 ...
 * ```
 */
export function markChunkStart() {
...
/**
 * Derives the high-level state from the raw extension status booleans.
 */
function deriveExtensionState(isBroadcasting, isExtensionRunning, isCapturingChunk) {
  if (!isBroadcasting) return 'idle';
  if (!isExtensionRunning) return 'starting';
  if (isCapturingChunk) return 'capturingChunk';
  return 'running';
}
...
export function getExtensionStatus() {
  ...
  const raw = NitroScreenRecorderHybridObject.getExtensionStatus();
  return {
    ...raw,
    state: deriveExtensionState(raw.isBroadcasting, raw.isExtensionRunning, raw.isCapturingChunk)
  };
}
