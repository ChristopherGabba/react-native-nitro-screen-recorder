...
/** Default timeout for waiting for global recording to start (2 minutes) */
const DEFAULT_GLOBAL_RECORDING_TIMEOUT_MS = 120000;
 ...
 * ```
 */
export async function startGlobalRecording(input) {
  ...
  const timeoutMs = input?.timeoutMs ?? DEFAULT_GLOBAL_RECORDING_TIMEOUT_MS;
  return NitroScreenRecorderHybridObject.startGlobalRecording(input?.options?.enableMic ?? false, input?.options?.separateAudioFile ?? false, timeoutMs);
}
